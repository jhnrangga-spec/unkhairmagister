...
export async function submitPendaftaranBaru(formData: FormData) {
  ...
  // FIX C.5: upload di luar try/catch khusus redirect,
  // tangkap error upload secara terpisah agar pesan error bisa tampil.
  ...
  if (uploadError) {
        redirect(`/pendaftaran-baru?error=${encodeURIComponent(uploadError)}`);
  }
...
export async function submitPendaftaranWisuda(formData: FormData) {
  ...
  if (uploadError) {
        redirect(`/pendaftaran-wisuda?error=${encodeURIComponent(uploadError)}`);
  }
